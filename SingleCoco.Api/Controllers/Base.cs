...
namespace SingleCoco.Api.Controllers
{
    ...
    {
        public JsonResult Json<T>(bool t, string d) where T : class
        {
            Message msg = new Message(t, d);
            return base.Json(msg);
        }

        public JsonResult Json<T>(T data) where T : class
        {
            SingleDataMessage<T> msg = new SingleDataMessage<T>(true, data);
            return base.Json(msg);
        }
        ...
        public JsonResult JsonToDataTables<T>(Tuple<int, int, int, IEnumerable<T>> d) where T : class
        {
            JQDataTableMessage<T> obj = new JQDataTableMessage<T>();
            obj.draw = d.Item1;
            //msg.PageSize = d.Item2;
            obj.recordsTotal = d.Item3;
            obj.data = d.Item4;
    ...
    {
        public Message(bool b, string d)
        {
            Status = b;
            Msg = d;
        }
    ...
    public class SingleDataMessage<T>
    {

        public SingleDataMessage(bool status, T value)
        {
            Status = status;
            Data = value;
        }
    ...
    public class Message<T> : TableMessage where T : class
    {
